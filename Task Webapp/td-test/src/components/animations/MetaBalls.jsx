...
export default function MetaBalls({
  ...
  useEffect(() => {
    ...
    const metaBallsUniform = Array.from(
      { length: 50 },
      () => new Vec3(0, 0, 0)
    );
    ...
    for (let i = 0; i < count; i++) {
      const h1 = hash31(i + 1);
      const start = h1[0] * (Math.PI * 2);
      const dtFactor = 0.1 * Math.PI + h1[1] * (0.4 * Math.PI - 0.1 * Math.PI);
      const baseScale = orbitMin + h1[1] * (orbitMax - orbitMin);
      const h2 = hash33(h1);
      const toggle = Math.floor(h2[0] * 2.0);
      const radius = ballSizeMin + h2[2] * (ballSizeMax - ballSizeMin);
      ballParams.push({ start, dtFactor, baseScale, toggle, radius });
    }
    ...
    function update(t) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        metaBallsUniform[i].set(posX, posY, p.radius * ballScale); // <-- apply size scaling here
      }

      // cursor blob
      const cx = gl.canvas.width * 0.5,
        cy = gl.canvas.height * 0.5;
      const rx = gl.canvas.width * 0.15,
        ry = gl.canvas.height * 0.15;
      const targetX = pointerInside
        ? pointerX
        : cx + Math.cos(elapsed * speed) * rx;
      const targetY = pointerInside
        ? pointerY
        : cy + Math.sin(elapsed * speed) * ry;
    ...
    ballSizeMin,
    ballSizeMax,
    ballScale, // include new controls
  ]);
