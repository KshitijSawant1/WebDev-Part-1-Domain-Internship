...
const AddNewTaskModal = ({ open, onClose, onCreate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // Start time when the task is created
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              value={form.labels}
              onChange={handleChange}
              placeholder="Frontend , Urgent , WebDev"
              className="mt-1 w-full rounded-md border border-gray-300 dark:border-gray-700
            bg-white dark:bg-gray-800 px-3 py-2 text-sm outline-none focus:ring-2 
            focus:ring-blue-500"
            />
            {!!labelTokens.length && (
              <div className="mt-2 flex flex-wrap gap-2">
                {labelTokens.map((l, i) => (
                  <span
                    key={`${l}-${i}`}
                    className="text-[10px] font-semibold px-2 py-1 rounded bg-blue-100 text-blue-800 
         dark:bg-blue-900 dark:text-blue-100"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              End Date & Time<span className="text-red-300">*</span>
            </label>
            <input
              type="datetime-local"
              name="end_date"
          ...
          {error && (
            <p
              className=" text-sm font-medium text-red-700 bg-red-100 border-red-300
