...
const deriveInsights = (tasks) => {
  ...
  return {
    pie: [
      ...
      { name: "InProgress", value: status.inProgress, color: "#f59e0b" },
    ...
    stats: {
      total: tasks.length,
      compeleteThisWeek: tasks.filter(
        (t) =>
          t.end_date &&
...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 max-w-6xl mx-auto px-4 pt-18">
      <div className="p-5" />
      <div
        className="bg-white shadow-lg rounded-lg p-6 flex flex-cols sm:flex-row 
        sm:justify-between"
      >
        <div>
        ...
        {["Total Tasks", "Completed This Week", "Past"].map((t, i) => (
          <div key={i} className="bg-white rounded-lg shadow p-4">
            ...
            <p className="text-2l font-semibold">
              {loading
                ? "-"
                : [stats.total, stats.compeleteThisWeek, stats.overdue]}
            </p>
          </div>
        ))}
      </div>
